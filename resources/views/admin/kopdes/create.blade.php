...
@section('title', 'Tambah KopDes Baru')
...
<div class="max-w-xl mx-auto space-y-4">
    ...
    <div class="flex items-center gap-2">
        ...
        <div>
            <h1 class="text-lg md:text-xl font-bold text-gray-800 tracking-tight">Tambah KopDes</h1>
            <p class="text-xs text-gray-400 mt-0.5">Masukkan informasi koperasi desa baru</p>
        </div>
    </div>

    <!-- Form Card (Compact) -->
    <div class="bg-white rounded-xl shadow-xs border border-gray-150 p-4 sm:p-5">
        <form method="POST" action="{{ route('kopdes.store') }}" class="space-y-4">
            @csrf

            <!-- Nama KopDes -->
            <div>
                <label for="nama_kopdes" class="block text-xs font-semibold text-gray-600 mb-1.5">Nama KopDes</label>
                <input type="text" name="nama_kopdes" id="nama_kopdes" value="{{ old('nama_kopdes') }}" required
            ...
            <div>
                ...
                <textarea name="alamat" id="alamat" rows="3" required
                          ...
                          placeholder="Masukkan alamat lengkap kantor koperasi desa...">{{ old('alamat') }}</textarea>
                @error('alamat')
                <p class="text-[10px] text-red-500 mt-1">{{ $message }}</p>
                @enderror
            </div>
