...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <h1 class="text-2xl font-bold text-gray-800">Daftar KopDes</h1>
            <p class="text-sm text-gray-500 mt-1">Kelola data Koperasi Desa aktif dan nonaktif</p>
        </div>
        <div>
    ...
    <div class="bg-white rounded-2xl shadow-sm p-4 border border-gray-100">
        <form method="GET" action="{{ route('admin.kopdes') }}" class="flex flex-col sm:flex-row gap-3">
            <div class="relative flex-1">
                ...
                <input type="text" name="search" value="{{ $search }}" placeholder="Cari nama, email, telepon, alamat..." 
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-100">
                <thead class="bg-gray-50">
                    <tr>
                        <th class="px-6 py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Nama KopDes</th>
                ...
                <tbody class="divide-y divide-gray-100 bg-white">
                    ...
                    <tr class="hover:bg-gray-50 transition duration-100">
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-semibold text-gray-800">{{ $item->nama_kopdes }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">{{ $item->email }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">{{ $item->no_telp }}</td>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-center font-medium">
                            <div class="flex items-center justify-center gap-2">
                                <a href="{{ route('kopdes.edit', $item->id_kopdes) }}" 
                                   class="text-blue-600 hover:text-blue-900 bg-blue-50 hover:bg-blue-100 px-3 py-1.5 rounded-lg transition duration-150">
                                    Edit
                                </a>
                                <form action="{{ route('kopdes.destroy', $item->id_kopdes) }}" method="POST" 
                                      onsubmit="return confirm('Apakah Anda yakin ingin menghapus data KopDes ini?')">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" 
                                            class="text-red-600 hover:text-red-900 bg-red-50 hover:bg-red-100 px-3 py-1.5 rounded-lg transition duration-150">
                                        Hapus
                                    </button>
                                </form>
                            </div>
