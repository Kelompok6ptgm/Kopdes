...
<div class="max-w-xl mx-auto space-y-4">
    ...
    <div class="flex items-center gap-2">
        ...
        <div>
            <h1 class="text-lg md:text-xl font-bold text-gray-800 tracking-tight">Tambah Akun Manager</h1>
            <p class="text-xs text-gray-400 mt-0.5">Buat akun pengelola dengan hak akses level Manager</p>
        </div>
    </div>

    <!-- Form Card (Compact) -->
    <div class="bg-white rounded-xl shadow-xs border border-gray-150 p-4 sm:p-5">
        <form method="POST" action="{{ route('manager.store') }}" class="space-y-4">
            @csrf
            ...
            <!-- Password -->
            <div>
                <label for="password" class="block text-xs font-semibold text-gray-600 mb-1.5">Password Akun</label>
                <input type="password" name="password" id="password" required minlength="8"
                       class="w-full px-3 py-1.5 bg-gray-50 border border-gray-200 rounded-lg focus:outline-none focus:ring-1 focus:ring-red-500 focus:border-red-500 text-xs transition @error('password') border-red-500 focus:ring-red-500 @enderror"
