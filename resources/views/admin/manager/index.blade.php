...
<div class="space-y-4">
    ...
    @if($errors->any())
    <div class="bg-red-50 border border-red-200 text-red-700 px-4 py-2.5 rounded-xl text-xs font-semibold shadow-xs" role="alert">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
            <h1 class="text-xl md:text-2xl font-bold text-gray-800 tracking-tight">Daftar Akun Manager</h1>
            <p class="text-xs text-gray-400 mt-0.5">Kelola data dan hak akses para Manager Koperasi Desa</p>
        </div>
        <div>
            <a href="{{ route('manager.create') }}" 
               class="inline-flex items-center gap-1.5 bg-red-600 hover:bg-red-700 text-white font-bold px-4 py-2 rounded-lg shadow-xs transition duration-150 text-xs">
    ...
    <div class="bg-white rounded-xl shadow-xs border border-gray-150 p-3">
        <form method="GET" action="{{ route('admin.manager') }}" class="flex flex-col sm:flex-row gap-2">
    ...
    <div class="bg-white rounded-xl shadow-xs border border-gray-150 overflow-hidden">
            ...
            <table class="min-w-full divide-y divide-gray-150">
                ...
                <tbody class="divide-y divide-gray-150 bg-white text-xs text-gray-600">
                    ...
                    <tr class="hover:bg-gray-50/75 transition-colors">
                        <td class="px-4 py-2.5 font-bold text-gray-800">{{ $item->nama }}</td>
                        ...
                        <td class="px-4 py-2.5 text-right font-medium">
                            <div class="flex items-center justify-end gap-1.5">
                                <button type="button" 
                                        ...
                                        class="text-amber-600 hover:text-amber-900 bg-amber-50 hover:bg-amber-100 px-2 py-1 rounded-md transition duration-150 text-[11px] font-semibold cursor-pointer">
                                    Reset PW
                                </button>
                                <a href="{{ route('manager.edit', $item->id_user) }}" 
                                   class="text-blue-600 hover:text-blue-900 bg-blue-50 hover:bg-blue-100 px-2 py-1 rounded-md transition duration-150 text-[11px] font-semibold">
                                    Edit
                                </a>
                                <form action="{{ route('manager.destroy', $item->id_user) }}" method="POST" 
                                      class="inline" onsubmit="return confirm('Apakah Anda yakin ingin menghapus akun Manager ini?')">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" 
                                            class="text-red-600 hover:text-red-900 bg-red-50 hover:bg-red-100 px-2 py-1 rounded-md transition duration-150 text-[11px] font-semibold cursor-pointer">
                                        Hapus
                                    </button>
                                </form>
                            </div>
        ...
        <div class="px-4 py-2.5 bg-gray-50 border-t border-gray-150">
            {{ $managers->links() }}
        </div>
        @endif
    </div>
